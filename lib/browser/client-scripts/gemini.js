...
function prepareScreenshotUnsafe(selectors, opts) {
    ...
    if (opts.captureArea) {
        if (!opts.captureArea.top) {
            opts.captureArea.top = Math.round((rect.height - opts.captureArea.height) / 2);
        }
        if (!opts.captureArea.left) {
            opts.captureArea.left = Math.round((rect.width - opts.captureArea.width) / 2);
        }
        rect = new Rect({
            top: rect.top + opts.captureArea.top,
            left: rect.left + opts.captureArea.left,
